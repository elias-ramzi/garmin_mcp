...
# =============================================================================
# In-place editing (update_workout / update_workouts / replace_workout)
# =============================================================================

def _editable_workout_dto():
    """DTO in the shape Garmin returns, with globally-numbered stepOrder."""
    return {
        "workoutId": 123456,
        "workoutName": "Tempo Run",
        "estimatedDurationInSecs": 1800,
...
def _put_payload(mock_garmin_client):
    """Return the workout body sent to Garmin by the last PUT."""
    args, kwargs = mock_garmin_client.client.put.call_args
    return args, kwargs["json"]


@pytest.mark.asyncio
async def test_update_workout_puts_full_dto_to_same_id(app_with_workouts, mock_garmin_client):
    """update_workout edits in place: same id, PUT (not POST), full body"""
    import json as json_module

    mock_garmin_client.get_workout_by_id.return_value = _editable_workout_dto()

    result = await app_with_workouts.call_tool(
        "update_workout",
        {"workout_id": 123456, "changes": {
            "name": "Tempo Run 30",
    ...
    args, body = _put_payload(mock_garmin_client)
    assert args[0] == "connectapi"
    assert args[1] == "/workout-service/workout/123456"
    assert body["workoutName"] == "Tempo Run 30"
    assert body["workoutSegments"][0]["workoutSteps"][1]["endConditionValue"] == 1800.0
    # A new workout must not be created: the id and its calendar entries survive
    mock_garmin_client.upload_workout.assert_not_called()
    mock_garmin_client.delete_workout.assert_not_called()


@pytest.mark.asyncio
async def test_update_workout_omits_derived_estimates(app_with_workouts, mock_garmin_client):
    """The stale duration is dropped so Garmin recomputes it from the steps"""
    mock_garmin_client.get_workout_by_id.return_value = _editable_workout_dto()

    await app_with_workouts.call_tool(
        "update_workout",
        {"workout_id": 123456, "changes": {"steps": [{"order": 2, "end_condition_value": 2400}]}},
    )

    _, body = _put_payload(mock_garmin_client)
    assert "estimatedDurationInSecs" not in body


@pytest.mark.asyncio
async def test_update_workout_preserves_untouched_steps(app_with_workouts, mock_garmin_client):
    """Only the named step changes; the rest round-trips untouched"""
    mock_garmin_client.get_workout_by_id.return_value = _editable_workout_dto()

    await app_with_workouts.call_tool(
        "update_workout",
        {"workout_id": 123456, "changes": {"steps": [{"order": 2, "target_zone": 4}]}},
    )

    _, body = _put_payload(mock_garmin_client)
    warmup, interval = body["workoutSegments"][0]["workoutSteps"]
...
@pytest.mark.asyncio
async def test_update_workout_rejects_training_plan_uuid(app_with_workouts, mock_garmin_client):
    """Garmin Coach workouts are generated by Garmin and cannot be edited"""
    import json as json_module

    result = await app_with_workouts.call_tool(
        "update_workout",
        {"workout_id": "fd3d0a6b-1234-4c1e-9c2a-aaaabbbbcccc", "changes": {"name": "Mine"}},
    )

    result_data = json_module.loads(result[0][0].text)
    assert result_data["status"] == "failed"
    assert "cannot be edited" in result_data["message"]
    mock_garmin_client.client.put.assert_not_called()


@pytest.mark.asyncio
async def test_update_workout_does_not_put_on_invalid_change(app_with_workouts, mock_garmin_client):
    """A rejected change spec never reaches Garmin"""
    import json as json_module
    ...
    result = await app_with_workouts.call_tool(
        "update_workout",
        {"workout_id": 123456, "changes": {"steps": [{"order": 99, "description": "nope"}]}},
    )

    result_data = json_module.loads(result[0][0].text)
    assert result_data["status"] == "failed"
    assert "no step with order 99" in result_data["message"]
    mock_garmin_client.client.put.assert_not_called()


@pytest.mark.asyncio
async def test_update_workouts_reports_each_result(app_with_workouts, mock_garmin_client):
    """Batch updates continue past a failure and report every outcome"""
    import json as json_module
    ...
    result = await app_with_workouts.call_tool(
        "update_workouts",
        {"updates": [
            {"workout_id": 123456, "changes": {"name": "First"}},
            {"workout_id": 222222, "changes": {"steps": [{"order": 99, "description": "x"}]}},
            {"workout_id": 333333},
        ]},
    )
    ...
    assert result_data["results"][0]["status"] == "success"
    assert "no step with order 99" in result_data["results"][1]["message"]
    assert "missing a 'changes' object" in result_data["results"][2]["message"]


@pytest.mark.asyncio
async def test_replace_workout_overwrites_keeping_id(app_with_workouts, mock_garmin_client):
    """replace_workout swaps the whole structure but keeps the workout id"""
    import json as json_module

    mock_garmin_client.get_workout_by_id.return_value = _editable_workout_dto()
    new_workout = _running_workout_with_steps([
        {
            "type": "ExecutableStepDTO",
            "stepOrder": 1,
            "stepType": {"stepTypeId": 3, "stepTypeKey": "interval"},
    ...
    result = await app_with_workouts.call_tool(
        "replace_workout",
        {"workout_id": 123456, "workout_data": new_workout},
    )
    ...
    args, body = _put_payload(mock_garmin_client)
    assert args[1] == "/workout-service/workout/123456"
    # Garmin reads the id from the URL; the body is aligned so logs agree
    assert body["workoutId"] == 123456
    assert body["workoutName"] == "Rebuilt"
    assert len(body["workoutSegments"][0]["workoutSteps"]) == 1
    mock_garmin_client.upload_workout.assert_not_called()


@pytest.mark.asyncio
async def test_replace_workout_rejects_partial_body(app_with_workouts, mock_garmin_client):
    """Garmin 400s on a body without segments, so it is caught before the call"""
    import json as json_module

    result = await app_with_workouts.call_tool(
        "replace_workout",
        {"workout_id": 123456, "workout_data": {"workoutName": "Name only"}},
    )

    result_data = json_module.loads(result[0][0].text)
    assert result_data["status"] == "failed"
    assert "workoutSegments" in result_data["message"]
    mock_garmin_client.client.put.assert_not_called()


@pytest.mark.asyncio
async def test_replace_workout_validates_target_types(app_with_workouts, mock_garmin_client):
    """The upload_workout validations apply to a replace as well"""
    import json as json_module

    bad = _running_workout_with_steps([
        _timed_interval_step({"workoutTargetTypeId": 2, "workoutTargetTypeKey": "power.between"}),
    ])

    result = await app_with_workouts.call_tool(
        "replace_workout",
        {"workout_id": 123456, "workout_data": bad},
    )
